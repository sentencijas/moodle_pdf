...
// принимает MoodleCourse и генерирует PDF
public class MoodlePdfGenerator {
    //мапа правильных ответов
    private final Map<String, String> correctAnswers = new LinkedHashMap<>();

    public void generatePdfFromCourse(MoodleCourse courseData, String outputPath, String fontPath) throws Exception {
    ...
    private float addRadioQuestion(PdfDocument pdf, Document doc, PdfAcroForm form,
        ...
        PdfButtonFormField radioGroup = PdfFormField.createRadioGroup(pdf, fieldName, null);
    ...
    private String generateCheckScript(int questionCount) {
        ...
        for (int i = 1; i <= questionCount; i++) {
            ...
            js.append("  results += 'Вопрос " + i + ": правильно\\n';\n");
            js.append("} else {\n");
            js.append("  results += 'Вопрос " + i + ": ошибка\\n';\n");
            js.append("}\n");
        }

        js.append("app.alert(results + '\\nИтог: ' + score.toFixed(2) + '%', 3);");
        return js.toString();
    }
}
